...
#from tensorflow.keras import PReLU
from tensorflow.keras.layers import Conv2D, Conv2DTranspose, UpSampling2D, ZeroPadding2D
from tensorflow.keras.layers import SpatialDropout2D, Permute, Activation, Reshape
...
def en_build(inp, dropout_rate=0.01):
    ...
    enet = tf.keras.layers.PReLU(shared_axes=[1, 2])(enet)
...
def de_bottleneck(encoder, output, upsample=False, reverse_module=False):
    ...
    if not upsample:
        ...
        x = UpSampling2D(2)(x)
        x = Conv2D(internal, (3,3), strides=(1, 1), padding='same')(x)
...
def de_build(encoder, nc):
    ...
    enet = UpSampling2D(2)(enet)
    enet = Conv2D(filters=nc, kernel_size=(2, 2), strides=(1, 1), padding='same')(enet)
