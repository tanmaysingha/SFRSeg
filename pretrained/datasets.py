...
def augment(image, label, target_size=(1024, 1024, 3), classes=19):
    ...
    #newly added augmentation
    #image_transformed = tf.image.random_hue(image_transformed, 0.2) #newly added
    #image_transformed = tf.image.random_contrast(image_transformed, 0.1, 0.5) #newly added
    #image_transformed = tf.image.random_saturation(image_transformed, lower=0.5, upper=1.75)
    ...
    label_transformed = tf.clip_by_value(label_transformed, 0, classes)
...
def parse_function(image, label, target_size=(1024, 1024, 3), training=True, classes=19):
    ...
    # Otherwise, its a test or validation set
        ...
        # Clip the 255 values down to 19
        label = tf.clip_by_value(label, 0, classes)
...
def create_dataset(cityscapes_root, batch_size, epochs, target_size, train=True, coarse=False, classes=19,
    ...
    if train and not coarse:
        #training with coarse and train extra datasets
