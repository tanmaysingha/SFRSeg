...
from functools import reduce #for reduce function in separable conv block
from tensorflow.keras.metrics import Metric as mx
...
parser.add_argument(
    "-m", "--model",
    help="Specify the model you wish to use: OPTIONS: SFRSeg, FANet, DFANet, ICNet, fastscnn",
    choices=['ICNet', 'fastscnn', 'FANet','DFANet', 'SFRSeg','ContextNet','STDC1_Seg','STDC2_Seg'],
    required=True)
parser.add_argument(
    "-r", "--resume",
...
if args.resume is not None and os.path.isfile(args.resume):
    ...
    initial_epoch = int(filename_split[1])
...
class csMeanIoU(mx):
    ...
    def __init__(self, num_classes, name=None, dtype=None):
        ...
        self.cm = self.add_weight(
            'confusion_matrix',
            shape=(num_classes - 1, num_classes - 1),#shape=(num_classes - 1, num_classes - 1),
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        ...
        curr_cm = tf.math.confusion_matrix(
            y_true, y_pred, self.num_classes, dtype=tf.float64) # float32 didn't work
    ...
    def result(self):
        ...
        #added the following lines here instead of keeping it below
        # followed deeplab v3 GitHub: https://github.com/rishizek/tensorflow-deeplab-v3/blob/master/evaluate.py
        num_valid_entries = tf.math.reduce_sum(
            tf.cast(tf.math.not_equal(denom, 0), dtype=tf.float64)) # float64 is replaced by 32
...
if model_name == 'ICNet':
    ...
    model = ICNet.build_bn(2048, 1024, 19, weights_path=None, train=False)
...
# Else we are training a model
    ...
    class prediction_on_epoch(keras.callbacks.Callback):
        ...
        def on_epoch_end(self, epoch, logs=None):
            image_to_predict = CITYSCAPES_ROOT + \
                '/leftImg8bit/val/munster/munster_000000_000019_leftImg8bit.png'
            ...
            prediction = tf.reshape(tf.argmax(prediction, axis=-1),(target_size[0], target_size[1], 1))
            
            # Now do matrix multiply
            prediction = np.matmul(tf.one_hot(prediction, CLASSES-1), CITYSCAPES_IDS)

            semantic_map = []
            # Create boolean mapping layer for each class based on IDs (true for index corresponding to class, false for others) 
            for id in CITYSCAPES_IDS:
                class_map = tf.reduce_all(tf.equal(prediction, id), axis=-1)
                semantic_map.append(class_map)
                # Save the Image
            semantic_map = tf.stack(semantic_map, axis=-1)
            semantic_map = tf.cast(semantic_map, tf.float32)
            image_to_save = semantic_map.numpy()
            image_to_save = np.matmul(image_to_save, CITYSCAPES_COLORS)
    ...
    callbacks = [
        ...
        hvd.callbacks.LearningRateWarmupCallback(warmup_epochs=5, verbose=1, initial_lr=0.045), #replace 0.45 by 0.045
    ]
    ...
    if hvd.rank() == 0:
        callbacks = callbacks + [
            ...
            keras.callbacks.ModelCheckpoint(
                logs_dir+'/weights-{epoch:02d}-{val_accuracy:.2f}-{val_loss:.2f}-'+time+'.hdf5', 'val_meanIoU', mode='max', save_best_only=True),prediction_on_epoch(target_size)]
    # Set the number of steps per epoch
    t = len(datasets.get_cityscapes_files(CITYSCAPES_ROOT, 'leftImg8bit', 'train', 'leftImg8bit'))
    v = len(datasets.get_cityscapes_files(CITYSCAPES_ROOT, 'leftImg8bit', 'val', 'leftImg8bit'))
    
    training_steps = np.ceil((t) / batch_size) 
    
    validation_steps = np.ceil((v) / batch_size) 
    verbose = 0
    if hvd.rank() == 0:
        verbose = 1
